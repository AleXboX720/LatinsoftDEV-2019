...
class ServiciosController extends ConfiguracionController
{
	...
	public function imprimirNEW($servicio, $programadas)
	{
		$servicio = $servicio[0]->toArray();
		...
		try {
			...
			*/
			...
			if($servicio['serv_anter'] !== null){
				$serv_anter = date("H:i", strtotime($this->_zonaHoraria, $servicio['serv_anter']));
				$this->letra2($printer, $serv_anter ."\n");
			} else {
				$this->negrita2($printer, " N/A \n");
			}
